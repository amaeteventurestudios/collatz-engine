...
async function readRecordsPayload() {
  ...
  if (longest.error || highestPeak.error || peakRatioSample.error) {
    throw new Error("Unable to read catalog records.");
  }
...
export async function GET() {
  ...
  const ttlMs = COLLATZ_CACHE_TTL_MS.PUBLIC_RECORDS;
  ...
  try {
    ...
    logReadCacheDiagnostic("api/collatz/records", meta, startedAt, data);
