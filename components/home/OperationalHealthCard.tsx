...
export function OperationalHealthCard() {
  const [state, setEngineState] = useState<PublicHealthSnapshot | null>(null);

  const load = useCallback(async (signal: AbortSignal) => {
    const res = await fetch("/api/collatz/health", { signal });
    const json = (await res.json()) as PublicHealthSnapshot;
    if (!res.ok || !json.ok) throw new Error("Public health status unavailable.");
    setEngineState(json);
  }, []);

  useSafePolling({
    intervalMs: COLLATZ_POLL_MS.PUBLIC_HEALTH,
    minIntervalMs: 60_000,
    staleAfterMs: COLLATZ_POLL_MS.PUBLIC_HEALTH * 2,
    poll: load,
  });

  const throughput = Number(state?.numbersPerSecond ?? 0);
  const hasData = state !== null && (state.lastBatchSize ?? 0) > 0;

  const metrics = [
    {
      label: "Last Batch Size",
      value: hasData ? fmt(state?.lastBatchSize) : "Pending",
      sub: "numbers processed",
    },
    {
      label: "Batch Duration",
      value: hasData ? fmtMs(state?.lastBatchDurationMs) : "Pending",
      sub: "wall-clock time",
    },
    {
      label: "Throughput",
    ...
    },
    {
      ...
      value: state?.lastRunAt
        ? fmtTime(state.lastRunAt)
        : "Pending",
      sub: "batch completed at",
    },
    {
      label: "Status",
      value: state?.currentStatus ? state.currentStatus.toUpperCase() : "Pending",
      sub: "public health state",
    },
  ];
        ...
        {hasError && (
          <div className="mb-3 flex items-start gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2.5 dark:border-red-800 dark:bg-red-900/20">
            ...
            <p className="text-xs text-red-700 dark:text-red-300">
              <span className="font-semibold">Operational notice:</span>{" "}
              Engine reported an operational error. Details are available to administrators.
            </p>
          </div>
        )}
