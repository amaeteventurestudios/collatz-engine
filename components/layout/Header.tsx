    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/95">
      ...
      <div className="mx-auto hidden max-w-7xl items-center justify-between px-6 py-3 lg:flex">
        ...
        <div className="flex items-center gap-1">
          ...
          <Link
            href="/admin"
            className="flex h-9 items-center rounded-md border border-slate-200 px-2.5 text-[11px] font-medium text-slate-500 transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 dark:border-slate-700 dark:text-slate-500 dark:hover:border-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-300"
          >
            Admin
          </Link>
        </div>
      ...
      <div className="grid grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center gap-2 px-3 py-3 lg:hidden">
        {/* Col 1 – hamburger */}
        ...
        <Link
          href="/"
          className="mx-auto flex min-w-0 max-w-[12rem] items-center justify-center gap-1.5 text-center text-[13px] font-semibold leading-tight text-slate-900 dark:text-slate-100"
        >
          <EngineIcon size="sm" />
          <span className="truncate">The Collatz Engine</span>
        </Link>
      ...
      {menuOpen && (
        <nav
          className="border-t border-slate-200 bg-white px-4 pb-5 pt-2 dark:border-slate-800 dark:bg-slate-950 lg:hidden"
          ...
          <div className="flex flex-col gap-0.5 text-center">
            {navLinks.map((link) => (
              <Link
                ...
                className="flex items-center justify-center rounded-lg px-3 py-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100"
              >
                {link.label}
              </Link>
            ))}
            <div className="mt-1 border-t border-slate-100 pt-1 dark:border-slate-800">
              <Link
                href="/admin"
                onClick={() => setMenuOpen(false)}
                className="flex items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-xs font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:text-slate-500 dark:hover:bg-slate-800 dark:hover:text-slate-300"
              >
